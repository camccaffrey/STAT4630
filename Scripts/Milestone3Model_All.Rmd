...
```{r}
cost_of_living_final <- cost_of_living %>%
  rename(
    Rent = rent1.center,
    CheapMeal=CheapMeal,
    BreadLoaf=bread,
...
middle = median(train$Rent, na.rm=T)
train = train %>% mutate(Expensive = ifelse(Rent>=middle, "1", "0"))
train = na.omit(train[-c(1)])
train$Expensive <- as.factor(train$Expensive)

test = test %>% mutate(Expensive = ifelse(Rent>=middle, "1", "0"))
test = drop_na(test[-c(1)])
...
set.seed(4630)
LDA5CV<-ipred::errorest(Expensive ~ ., data=train, model=lda,
estimator="cv",
est.para=control.errorest(k=5),
predict=cv)$err
cat("The test error of 5-fold CV of LDA is", LDA5CV, "\n")
LDA10CV<-ipred::errorest(Expensive ~ ., data=train, model=lda,
estimator="cv",
...
```{r}
cost_of_living_final2 <- cost_of_living %>%
  rename(
    Country = country,
    Rent = rent1.center,
    CheapMeal=CheapMeal,
...
middle2 = median(train2$Rent, na.rm=T)
train2 = train2 %>% mutate(Expensive = ifelse(Rent>=middle, "1", "0"))
train2 = na.omit(train2[-c(2)])
train2$Expensive <- as.factor(train2$Expensive)

test2 = test2 %>% mutate(Expensive = ifelse(Rent>=middle, "1", "0"))
test2 = drop_na(test2[-c(2)])
test2$Expensive <- as.factor(test2$Expensive)

middle_toyota = median(train2$Toyota, na.rm=T)
train2 = train2 %>% mutate(Toyota = ifelse(Toyota>=middle_toyota, "1", "0"))
train2$Toyota <- as.factor(train2$Toyota)

test2 = test2 %>% mutate(Toyota = ifelse(Toyota>=middle_toyota, "1", "0"))
test2$Toyota <- as.factor(test2$Toyota)


train2 = train2 %>% mutate(Country = ifelse(Country=="United States", "1", "0"))
train2$Country <- as.factor(train2$Country)

test2 = test2 %>% mutate(Country = ifelse(Country=="United States", "1", "0"))
test2$Country <- as.factor(test2$Country)

```

Added in Coffee, McMeal, School, and Ticket as numerical variables and 
Toyota as a categorical variable(with levels 1> median, 0<median).

```{r}
cheap <- train2[which(train2$Expensive == 0),]
expensive <- train2[which(train2$Expensive == 1),]

ICS::mvnorm.kur.test(cheap[,2:12])
ICS::mvnorm.kur.test(expensive[,2:12])

ICS::mvnorm.skew.test(cheap[,2:13])
ICS::mvnorm.skew.test(expensive[,2:13])

#assumptions not met, interpretations may not be reliable??
```

```{r}
lr_train3 <-glm(Expensive~ CheapMeal + BreadLoaf + Eggs + MonthlySalary + Coffee + McMeal + School + Ticket + Toyota + Country, family = binomial, data = train2)
summary(lr_train3) 

#no eggs
lr_train4 <-glm(Expensive~ CheapMeal + BreadLoaf + MonthlySalary + Coffee + McMeal + School + Ticket + Toyota + Country, family = binomial, data = train2)
summary(lr_train4) 
```
...
lrtest(lr_train3, lr_train4)
#no sig diff when taking eggs out

anova(lr_train3)

TS1 <- lr_train$dev - lr_train3$dev
1-pchisq(TS1,5)
## ????
```
...
```{r}
# ROC Curve
lr_preds2<-predict(lr_train3,newdata=test2, type="response")
lr_rates2<-ROCR::prediction(lr_preds2, test2$Expensive)
lr_roc_result2<-ROCR::performance(lr_rates2,measure="tpr", x.measure="fpr")
plot(lr_roc_result2, main="ROC Curve for Logistic Regression")
```

## iii)
```{r}
lr_auc2<-ROCR::performance(lr_rates2, measure = "auc")
lr_AUC2<-lr_auc2@y.values

#AUC is 0.9605077

#previous AUC: 0.9591262 
```

## iv)
```{r}
set.seed(4630)
LR5CV2<-boot::cv.glm(train2,lr_train3, K=5) 
cat("The test error of 5-fold CV of Logistic Regression is", LR5CV2$delta[1], "\n")
LR10CV2<-boot::cv.glm(train2,lr_train3, K=10) 
cat("The test error of 10-fold CV of Logistic Regression is", LR10CV2$delta[1], "\n")
...
```{r}
label2<-as.numeric(lr_preds2>0.8)
lr_acc_test2<-mean(test2$Expensive == label2)
cat("The test error of logistic regression is", 1-lr_acc_test, "\n")

# originally 0.1161677 
```
...
In the original logistic regression model from part 3c, 